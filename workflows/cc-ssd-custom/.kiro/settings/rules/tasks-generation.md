...
- Documentation tasks (see Documentation Tasks section below)
...
When warranted (see Documentation Decision Process below), add a documentation task as the **last major task**.

## Documentation Decision Process

### Step 1: Check Triggers (defaults)
- New user-facing feature → `docs/features/feature-name.md`
- New or changed API endpoints → OpenAPI spec / API docs
- New config or environment variables → README or environment docs
- Architectural decision → `docs/adr/NNNN-title.md`

### Step 2: Apply Reasoning Gate

For each triggered doc (and any untriggered case), weigh all four tests:

1. **Necessity**: Attempt to complete this sentence — if it rings hollow, reconsider:
   "Without this doc, [who] will be confused about [what]."

2. **Redundancy**: If design.md + code tell the full story to someone
   *without access to the spec*, skip it.

3. **Form**: Pick the lightest viable option:
   Inline addition > ADR > Standalone doc
   (ADR only for non-obvious trade-offs future engineers will question)

4. **Maintainability**: If it requires manual updates each sprint,
   link to the source of truth instead of duplicating.

Tasks that pass the gate: `- [ ]` (required).
Tasks that fail but have marginal value: `- [ ]*` (optional).
Tasks that clearly fail: omit entirely.
