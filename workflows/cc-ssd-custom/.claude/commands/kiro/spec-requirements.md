...
1. **Load Context**:
   ...
   - If `spec.json.intentSpec` is `false`, output a visible warning block before proceeding:
     > ⚠️ **No intent exploration performed.** Requirements will be based on description only — user needs (JTBD, forces of progress) are ungrounded. Consider running `/kiro:intent-explore $1` first.
   - Read `.kiro/specs/$1/requirements.md` for project description
   - **Load ALL steering context**: Read entire `.kiro/steering/` directory including:
     - Default files: `structure.md`, `tech.md`, `product.md`
     - All custom steering files (regardless of mode settings)
     - This provides complete project memory and context
   - **Load intent exploration context (if exists)**: Read `.kiro/specs/$1/intent-spec.md`
     if present. Use JTBD, forces of progress, and problem statement to ground requirements
     in discovered user needs.
